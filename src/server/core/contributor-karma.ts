...
export const createContributorKarmaBuckets = (
  ...
  const knownContributors = contributors
    .flatMap((contributor) =>
      typeof contributor.subredditKarma === 'number' &&
      Number.isFinite(contributor.subredditKarma)
        ? [
            {
              contributorName: contributor.id,
              subredditKarma: contributor.subredditKarma,
            },
          ]
        : []
    )
    .sort(
      (a, b) =>
        a.subredditKarma - b.subredditKarma ||
  ...
  knownContributors.forEach((contributor, index) => {
    const bucket =
      maxContributorIndex === 0
        ...
        : Math.round(
            (index / maxContributorIndex) * (SUBREDDIT_KARMA_BUCKET_COUNT - 1)
          );

    contributorKarmaBuckets.set(
      contributor.contributorName,
      bucket as SubredditKarmaBucket
    );
  });
